...
class Tesseract():
    ...
    def __init__(self, lang='rus', tpath='tesseract'):
        self.tpath = tpath
    ...
    def from_image(self, image, basewidth=100, whitelist=None):
        if isinstance(image, Image.Image):
            ...
            if image.mode == 'RGBA':
                ...
                image = whiteimage #whiteimage.save('tmp.png')
            if image.mode != 'L':
        ...
        if isinstance(image, QImage):
            log.debug('process QImage')
            if not image.isGrayscale():
                image = image.convertToFormat(QImage.Format_RGB16, flags=Qt.MonoOnly)
            if image.width() < basewidth:
                image = image.scaledToWidth(basewidth)
            qb = QByteArray()
            for i in range(0, image.height()):
                qb = qb.append(image.scanLine(i).asstring(image.bytesPerLine()))
            w = image.width()
            h = image.height()
            depth = image.depth()
            buff_img = qb.data()
    ...
    def recognize(self, url, *args, **kwargs):
        if isinstance(url, Image.Image) or isinstance(url, QImage):
...
if __name__ == "__main__":
    ...
    os.environ['PATH'] = os.path.join(os.path.dirname(__file__),'tesseract') + ';' + os.environ['PATH']

    t = Tesseract()
    t.set_pagemode(TessPageSegMode.PSM_SINGLE_LINE)

    qimage = QImage()
    qimage.load('d:\\work\\cm2\\test\\tess\\avito.png')
    print t.recognize(qimage, 300, '0123456789-')
